...
export default function VersionsPage(): React.ReactElement {
  ...
  // Estimate unique sites (rough — actual unique count requires per-site data)
  const uniqueSiteEstimate = Math.max(
    ...allJobs.map((j) => j.totalSites),
    0
  );
      ...
      <div className="glass-card animate-fade-in-up rounded-xl p-6">
        {isLoading ? (
          ...
          <div className="text-center py-4">
            ...
            <p className="mt-3 text-lg font-semibold text-muted-foreground">No cleanup runs yet</p>
            <p className="mt-1 text-sm text-muted-foreground">
              Trigger your first version cleanup to start reclaiming storage space
            </p>
          </div>
        ) : (
          <>
            <div className="text-center">
              <p
                ...
                style={{ textShadow: "0 0 40px rgba(0, 201, 183, 0.3)" }}
              >
                {formatBytes(totalReclaimed)}
              </p>
              <p className="mt-1 text-base font-semibold tracking-wide">
                Total Reclaimed across {uniqueSiteEstimate > 0 ? uniqueSiteEstimate.toLocaleString() : totalSitesProcessed.toLocaleString()} sites
              </p>
            </div>
        ...
        {isLoading ? (
          ...
          <div className="glass-card rounded-xl p-8 text-center">
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              No version cleanup runs found{statusFilter !== "all" ? ` with status "${statusFilter}"` : ""}
            </p>
          </div>
        ) : (
          filtered.map((job) => (
